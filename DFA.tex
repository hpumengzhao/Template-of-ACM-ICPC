...
using namespace std;
const int N = 500;
int n,nx,ny,m,num,w[N][N],boy[N],gir[N],slack[N],pre[N],lx[N],ly[N];
bool vx[N],vy[N];
queue<int> q;
void Changematch(int k){
	while(pre[k]){
		boy[k]=pre[k];
		int ne=gir[pre[k]];
		gir[boy[k]]=k;
...
void Getmatch(int s){
	...
	for(int i=1;i<=n;i++) slack[i]=1e9,vx[i]=vy[i]=pre[i]=0;//初始化
	while(1){
		while(!q.empty()){
			int k=q.front();q.pop();
			vx[k]=1;
			int tmp;
			for(int i=1;i<=n;i++){//增广 
				if(vy[i]==0&&(tmp=lx[k]+ly[i]-w[k][i])<slack[i]){
					pre[i]=k;
					if(tmp==0){
						if(boy[i]==0){
							Changematch(i);return ;
						}
						vy[i]=1;q.push(boy[i]);
					}else{
						slack[i]=tmp;
		...
		int delta=1e9,where=0;
		for(int i=1;i<=n;i++) if(vy[i]==0&&slack[i]<delta) delta=slack[i],where=i;
		for(int i=1;i<=n;i++){
			if(vx[i]) lx[i]-=delta;
			if(vy[i]) ly[i]+=delta;
			else{
		...
		if(boy[where]==0){
			Changematch(where); return ;
		}
		vy[where]=1;q.push(boy[where]);
	} 
}
int main(){
	scanf("%d %d %d",&nx,&ny,&m);
